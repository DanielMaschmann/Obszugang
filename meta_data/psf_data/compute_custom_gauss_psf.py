...
band_list = ['F275W', 'F336W', 'F475W', 'F814W']
instrument_list = ['uvis', 'uvis', 'acs', 'acs']

instrument_pixel_size_list = [0.0397, 0.0397, 0.05, 0.05]
...
for band, instrument, instrument_pixel_size in zip (band_list, instrument_list, instrument_pixel_size_list):
    print(band)

    psf_fwhm_arcsec = PSFTools.get_obs_psf_fwhm(band=band, instrument=instrument)
    psf_fwhm_pix = psf_fwhm_arcsec / instrument_pixel_size
    print('psf_fwhm_arcsec ', psf_fwhm_arcsec)
    print('psf_fwhm_pix ', psf_fwhm_pix)

    std_orig_img_pix = helper_func.TransTools.gauss_fwhm2sig(fwhm=psf_fwhm_pix * distance_ratio)
