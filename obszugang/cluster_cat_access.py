...
class ClusterCatAccess:
    ...
    def __init__(self,
        ...
        self.phangs_hst_cluster_cat_data_path = (
            access_config.phangs_config_dict)['phangs_hst_cluster_cat_data_path']
        self.phangs_hst_cluster_cat_release = (
            access_config.phangs_config_dict)['phangs_hst_cluster_cat_release']
    ...
    def open_hst_cluster_cat(self, target, classify='human', cluster_class='class12', cat_type='obs'):
        ...
        instruments = ''
        if obs_info.hst_cluster_cat_obs_band_dict[target]['acs']:
            instruments += 'acs'
            if obs_info.hst_cluster_cat_obs_band_dict[target]['uvis']:
                instruments += '-uvis'
        else:
            instruments += 'uvis'
    ...
    def load_hst_cluster_cat(self, target_list=None, classify_list=None, cluster_class_list=None):
        ...
        if target_list is None:
            target_list = obs_info.hst_cluster_cat_target_list
    ...
    def get_quick_access(self, target='all', classify='human', cluster_class='class123',
        ...
        quick_access_dict_path = \
            (Path(access_config.phangs_config_dict['phangs_hst_cluster_cat_quick_access_path']) /
        ...
        if os.path.isfile(quick_access_dict_path) and not reload:
            ...
            if target == 'all':
                if self.phangs_hst_cluster_cat_sed_version == 'ground_based_ha':
                    ...
                    target_list = obs_info.hst_cluster_cat_hst_ha_target_list
                else:
    ...
    def load_ccd_hull(self, ccd_type='ubvi', cluster_region='ycl', classify='human'):
        ...
        hull = np.genfromtxt(Path(access_config.phangs_config_dict['phangs_hst_cluster_cat_data_path']) /
    ...
    def load_extend_phot_table(self, target):
        ...
        file_path = (Path(access_config.phangs_config_dict['phangs_hst_cluster_cat_extend_photo_path']) /
                     ('%s_IR4' % access_config.phangs_config_dict['phangs_hst_cluster_cat_extend_photo_ver']))

        file_name = ('Phot_%s_%s_IR4_class12human.fits' %
                     (access_config.phangs_config_dict['phangs_hst_cluster_cat_extend_photo_ver'],
    ...
    def load_extend_sed_table(self, target):
        ...
        file_path = Path(access_config.phangs_config_dict['phangs_hst_cluster_cat_extend_sed_fit_path'])
